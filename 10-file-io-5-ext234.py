...
Part 3: Extension of budget app (optional)

1) Use hashes instead of if/else statements in the main while loop that runs the program (e.g. {"o":open_file,"s":save_file,"a":append_data} - each value is a function name.)

2) When the user opens a file, store the filename so that the user does not have to re-type it in when saving

3) Allow users to define column names in the file
First Id have to ask the users if they want default headers or custom 
If custom, then probably makes sense to do a while loop and allow them to continue entering headers
until they indicate they are done
Once i have this array of strings, this has to become the first index of my data dictionary
My add items method will also have to change. It will need to ask for an input for each of these new headers
my read/save method might also have to change.

4) Allow users to add/remove columns in the csv file


'''
#to user hashes to call the function ill have to start by making the values into function variables 
#i will create another variable to store the file name for reuse 
#will need a var to hold the state of the budget i.e. true if it was previously saved
#if prev saved then i can reuse the file name I have.
#will need a array to store the headers and will prompt the user d
...
def ui_loop():
	commands = {"a": ("Add an Item",add_an_item), "s":("Show Items",show_items), "r":("Remove an item",remove_items), "q":"Quit the program", "w":("Save your budget to a file", write_csv), "i":("Import your budget from a file", read_csv)}
